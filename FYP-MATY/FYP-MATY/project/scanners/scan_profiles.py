...
from .scoring_policy import cvss_selection, ScoringPolicyError
...
TOOL_OPTIONS = [
    ...
    {'id':'windows_patch_inventory','label':'Windows local patch inventory','category':'Host Identity','purpose':'Load an operator-exported local JSON record containing whitelisted Windows product, build, architecture and installed-KB evidence; no credentials or remote session.','full':False},
    ...
    {'id':'ssh_audit_native','label':'Native SSH cryptographic enrichment','category':'Information Gathering','purpose':'Collect one SSH identification/KEXINIT exchange for algorithm evidence; no login attempts or credentials.','full':True},
    ...
    *,
    ...
    def setting_provenance(
        ...
        *,
        ...
        requested = supplied.get(key) if key in supplied else default
        ...
        return {
            ...
            'source': 'operator' if key in supplied else 'default',
            'clamped': changed,
            'reason': reason if changed else '',
        }

    advanced_provenance = {
        'command_timeout_seconds': setting_provenance(
            'command_timeout_seconds', command_timeout,
            int(defaults.get('command_timeout_seconds') or 600),
            policy_min=ADVANCED_BOUNDS['command_timeout_seconds'][0],
            policy_max=ADVANCED_BOUNDS['command_timeout_seconds'][1],
            reason='normalised_to_policy_bounds',
        ),
        'retry_failed_batches': setting_provenance(
            'retry_failed_batches', retry_failed,
            bool(defaults.get('retry_failed_batches', True)),
            reason='normalised_to_boolean',
        ),
        'retry_count': setting_provenance(
            'retry_count', retry_count,
            int(defaults.get('retry_count') or 1),
            policy_min=ADVANCED_BOUNDS['retry_count'][0],
            policy_max=ADVANCED_BOUNDS['retry_count'][1],
            reason='normalised_to_policy_bounds',
        ),
        'ports_per_batch': setting_provenance(
            'ports_per_batch', ports_per_batch,
            int(defaults.get('ports_per_batch') or micro_cfg.get('batch_size_target') or 5),
            policy_min=ADVANCED_BOUNDS['ports_per_batch'][0],
            policy_max=ADVANCED_BOUNDS['ports_per_batch'][1],
            reason='normalised_to_policy_bounds',
        ),
        'parallel_scanning': setting_provenance(
            'parallel_scanning', parallel,
            bool(defaults.get('parallel_scanning', False)),
            reason='normalised_to_boolean',
        ),
        'parallel_workers': setting_provenance(
            'parallel_workers', parallel_workers,
            int(defaults.get('parallel_workers') or 2),
            policy_min=ADVANCED_BOUNDS['parallel_workers'][0],
            policy_max=ADVANCED_BOUNDS['parallel_workers'][1],
            reason=(
                'forced_to_one_when_parallel_scanning_is_disabled'
                if not parallel else 'normalised_to_policy_bounds'
            ),
        ),
    }
    ...
    try:
        selected_cvss = cvss_selection(cvss_version)
    except ScoringPolicyError as exc:
        # Preserve the documented default only so the normalized object remains
        # structurally complete. The validation error prevents execution from
        # silently using a different CVSS version than the operator requested.
        selected_cvss = cvss_selection(None)
    ...
    return {
        ...
        'advanced_settings_provenance': advanced_provenance,
