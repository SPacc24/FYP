...
def test_nvd_enrichment_only_fills_unscored_rows_and_never_overwrites_cve_program_metric():
    ...
    # The second row still lacks CVSS 4.0, so NVD is queried to fill only that
    # missing metric without overwriting the published CVSS 3.1 value.
...
def test_client_report_sources_do_not_reintroduce_retired_cve_status_terms():
    ...
    for phrase in ('MITRE candidate references retained', 'Validated MITRE Reference', 'Key Exposure Indicators'):
