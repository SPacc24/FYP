...
def test_nvd_is_enrichment_only_in_candidate_pipeline():
    source = (SCANNERS / 'enumerator.py').read_text(encoding='utf-8')
    ...
    assert '_enrich_missing_cvss_from_nvd(cve_matches' in source
...
def test_main_page_replaces_old_scan_and_vulnerability_tables():
    ...
    assert 'Table 1 — Candidate CVE Review' in template
