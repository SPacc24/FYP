...
TACTIC_WEIGHTS = {
    ...
    'execution':             1.5,
    'persistence':           2.0,
    'privilege-escalation':  2.5,  # Most critical — attacker gains full control
    'defense-evasion':       1.5,
    'credential-access':     2.5,  # Most critical — enables lateral movement
    'discovery':             0.5,  # Low impact alone, but enables other attacks
    'lateral-movement':      2.0,
    'collection':            1.5,
    'command-and-control':   2.0,
    'exfiltration':          2.5,  # Most critical — data loss
    'impact':                2.0,
}

DEFAULT_TACTIC_WEIGHT = 1.0

# ── CVE severity bands → score contribution ─────────────────────────────────
CVE_SEVERITY_CONTRIBUTION = {
    'critical': 3.0,
    'high':     2.0,
    'medium':   1.0,
    'low':      0.3,
...
def get_risk_label(score: float) -> dict:
    """Convert numeric score to label + colour for UI display."""
    if score >= 8.0:
...
class RiskScorer:
    ...
    def calculate(
        ...
        att_component  = self._score_attack(operation_results)
        combined       = self._combine(cve_component, att_component)

        label_info = get_risk_label(combined)

        breakdown = {
            'cve_component':    round(cve_component, 2),
            'attack_component': round(att_component, 2),
        ...
        log.info(
            f"Risk score: {combined:.2f} | "
            f"CVE={cve_component:.2f} ATT&CK={att_component:.2f}"
        )
    ...
    def _score_cve(self, vulnerabilities: list) -> float:
        ...
        scores = sorted(
            [v.get('cve_score', 0.0) for v in vulnerabilities],
            reverse=True
        )
    ...
    def get_remediation_for_technique(self, technique_id: str) -> dict:
        ...
        # Match on partial technique ID (e.g. T1021.002 → T1021)
    ...
    def get_all_remediations(self, operation_results: dict) -> list:
        """
        Get remediation advice for all SUCCESSFUL techniques in an operation.
        Only returns advice for techniques that actually succeeded.
        """
        remediations = []
        seen = set()
