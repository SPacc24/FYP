...
def test_layer_decision_ui_processes_one_path_at_a_time():
    template = (PROJECT_ROOT / "templates/layer_decision.html").read_text(encoding="utf-8")
    assert 'type="radio" name="path_id"' in template
    assert 'value="follow_path"' in template
    assert 'value="assess_current"' in template
    assert 'value="refresh_paths"' in template
    assert 'value="retry_layer"' in template
    assert 'value="stop"' in template
    assert 'value="revisit_segment"' in template
    assert 'value="authorize_route"' in template
    assert 'value="revoke_route"' in template
    assert 'name="allow_inconclusive_operator_targets"' in template
    assert 'name="route_observation_id"' in template
    assert 'name="segment_id"' in template
    assert "Select one authorised continuation path" in template


def test_phase3_backend_accepts_only_current_layer_checkbox_targets():
    ...
    assert 'workflow.get("current_segment_id")' in source
    assert 'current_segment.get("hosts")' in source
...
def test_discovery_module_does_not_create_routes_or_addresses():
    ...
    assert ".is_private" not in source
