...
def assess_windows_cve_patch(
    ...
    if not inventory.get("ok"):
        return {**result, "patch_state": "Insufficient patch evidence", "patch_basis": "Authenticated Windows patch inventory was not available."}
    if not applicable:
...
def enrich_windows_patch_states(
    ...
    for row in cve_rows:
        ...
        if not inventory:
            ...
            row["patch_evidence"] = {"patch_basis": "No authenticated Windows patch inventory was collected for this host."}
            continue
