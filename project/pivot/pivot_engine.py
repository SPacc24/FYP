...
class PivotEngine:
        ...
        try:
            ...
            self._chisel_process = subprocess.Popen(
                [CHISEL_BINARY, "server", "-p", str(port),
                 "--reverse", "--socks5", f"--socks5-port={socks_port}"],
    ...
    def configure_proxychains(self, socks_port: int = 1080) -> bool:
        """Write proxychains config to use our SOCKS proxy."""
        conf_path = Path(PROXYCHAINS_CONF)
        if not conf_path.exists():
            log.warning(f"{PROXYCHAINS_CONF} not found, creating it")
            conf_path.parent.mkdir(parents=True, exist_ok=True)

        content = conf_path.read_text()
        # Find [ProxyList] and set the line
        lines = content.split("\n")
        new_lines = []
        proxy_set = False
        in_proxy_list = False
        ...
        for line in lines:
            stripped = line.strip()
            if stripped.startswith("[ProxyList]"):
                in_proxy_list = True
            ...
            if in_proxy_list:
                if stripped.startswith("["):
                    in_proxy_list = False
                    new_lines.append(line)
                elif stripped.startswith("socks"):
                    if not proxy_set:
                        new_lines.append(f"socks5 127.0.0.1 {socks_port}")
                        proxy_set = True
                    continue
                elif not stripped:
                    new_lines.append(line)
        ...
        if not proxy_set:
            new_lines.append(f"[ProxyList]\nsocks5 127.0.0.1 {socks_port}\n")

        conf_path.write_text("\n".join(new_lines))
        log.info(f"✅ Proxychains configured: socks5 127.0.0.1 {socks_port}")
        return True
